...
const WHITELIST = new Set([
  ...
  'Number', 'String', 'Boolean', 'Date', 'Promise', 'Set', 'Map', 'Symbol', 'RegExp',
...
function addDestructured(target, group) {
  group.split(',').forEach((part) => {
    ...
    const id = right.split('=')[0].trim().replace(/^\.\.\./, '')
...
export function findUndeclaredValueRefs(raw) {
  ...
  const scan = (re, fn) => { let m; while ((m = re.exec(code))) fn(m) }
...
function selfTest() {
  const cases = [
    { name: '抓未声明引用', code: 'const props = defineProps({ a: String })\nconst x = ref(1)\nx.value\nfoo.value\n', expect: ['foo'] },
    { name: '注释里的示例不算', code: '// 别写 moduleTitle.value\n/* bar.value */\nconst x = ref(1)\nx.value\n', expect: [] },
    { name: '解构/参数/导入正确识别', code: "import { ref } from 'vue'\nconst { a: b } = props\nconst f = (row) => row.value\nconst g = ref(0)\ng.value\nb.value\n", expect: [] },
...
function main() {
  const sfc = require(path.join(FRONTEND, 'node_modules', '@vue', 'compiler-sfc'))
  const files = walk(path.join(FRONTEND, 'src')).filter((f) => f.endsWith('.vue'))
  let hits = 0
  for (const f of files) {
    const src = fs.readFileSync(f, 'utf8')
    let descriptor
    try { descriptor = sfc.parse(src, { filename: path.basename(f) }).descriptor } catch (e) { continue }
