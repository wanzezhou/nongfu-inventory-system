<template>
  <div class="other-incomes">
    <!-- 筛选区（与「其他支出」页同构：时间范围 + 类别 + 关键词 + 操作按钮组） -->
    ...
    <div v-if="sumAmount > 0" class="sum-bar">
      当前筛选合计：<span class="sum-text">¥{{ fmtMoney(sumAmount) }}</span
      >（共 {{ total }} 条）
    </div>
    ...
    <el-card class="table-card" shadow="never">
      <el-table :data="rows" v-loading="loading" border stripe size="small">
        <el-table-column prop="incomeName" label="收入名称" min-width="160" show-overflow-tooltip />
        <el-table-column prop="category" label="收入类别" width="110">
        ...
        <el-table-column prop="amount" label="金额" width="120" align="right">
          <template #default="{ row }"
            ><span class="amount-text">¥{{ fmtMoney(row.amount) }}</span></template
          >
        </el-table-column>
        <el-table-column prop="incomeDate" label="收入日期" width="110" />
        <el-table-column prop="accountName" label="收入账户" width="110">
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="90px">
        <el-form-item label="收入名称" prop="incomeName">
          <el-input v-model="form.incomeName" placeholder="如：废纸箱出售、银行利息" maxlength="200" />
        </el-form-item>
        <el-form-item label="金额" prop="amount">
          <el-input-number
            ...
            :step="100"
            style="width: 100%"
            placeholder="大于 0 的金额"
          />
        </el-form-item>
        <el-form-item label="收入日期" prop="incomeDate">
          <el-date-picker v-model="form.incomeDate" type="date" value-format="YYYY-MM-DD" style="width: 100%" />
        </el-form-item>
    ...
    <el-dialog v-model="importResultVisible" title="导入结果" :width="dialogWidth">
      ...
      />
      <el-table
        v-if="importResult.failed && importResult.failed.length"
...
const fetchList = async () => {
  ...
  loading.value = true
  try {
    const res = await getIncomes(buildParams())
    if (res.data) {
...
const fetchMeta = async () => {
  try {
    const [c, a] = await Promise.all([getIncomeCategories(), getFinanceAccounts()])
...
const openForm = row => {
  ...
  if (row) {
    form.id = row.incomeId
    form.incomeName = row.incomeName
    form.amount = row.amount
    form.incomeDate = row.incomeDate
...
const submitForm = async () => {
  ...
  try {
    ...
    delete payload.id
    if (form.id) {
      await updateIncome(form.id, payload)
...
const handleDelete = async row => {
  try {
    await ElMessageBox.confirm(
      `确定删除「${row.incomeName}」（¥${fmtMoney(row.amount)}）？${row.accountName ? '将同步撤销账户流水并回退余额。' : ''}`,
      '删除确认',
  ...
  try {
    await deleteIncome(row.incomeId)
...
const downloadTemplate = async () => {
  try {
    ...
    downloadBlob(res.data, '其他收入导入模板.xlsx')
  } catch (e) {
...
const onFileChange = async ev => {
  ...
  fd.append('file', file)
  try {
    const res = await importIncomes(fd)
...
const handleExport = async format => {
  try {
    ...
    const res = await exportIncomes(params)
    downloadBlob(res.data, `其他收入_${Date.now()}.${format === 'csv' ? 'csv' : 'xlsx'}`)
  } catch (e) {
...
/* 收入用 success 语义色，与其他支出（danger）形成一眼可辨的区分 */
.amount-text {
  color: var(--el-color-success);
  font-weight: 600;
}
