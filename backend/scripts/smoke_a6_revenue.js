 ...
 *      类型1/2(无抵扣·行内抵扣)/3/4 各分支数值断言；与财务营收明细列表（同口径）交叉验证。
...
(async () => {
  ...
  try {
    ...
    // ---------- 用例1：类型1 官方平台 → 营收=(10+2)×3=36，配送费拆分=6 ----------
    console.log('\n[类型1] 官方平台销售');
    ...
    for (const [oid, want, tag] of [[o1, 36, '类型1'], [o2a, 48, '类型2无抵扣'], [o2b, 72, '类型2抵扣'], [o3, 50, '类型3']]) {
      const row = rowOf(oid);
