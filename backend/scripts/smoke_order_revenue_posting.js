...
async function main() {
  ...
  try {
    ...
    await pool.query(`INSERT INTO inventory (product_id, quantity, updated_at) VALUES (?, 1000, NOW())`, [PROD_ID]);
    await pool.query(
    ...
    const bal = async type => {
      // ⚠️ 必须与生产**同序**：orderRevenuePosting.loadAccountByType 是 `ORDER BY created_at LIMIT 1`。
      //    原先这里没有 ORDER BY、直接取 r[0]（主键顺序），与生产可能**不是同一个账户** ——
      //    库里一旦存在同类型的额外账户（曾积了 56 个冒烟账户，ACCMUC… 的主键序在 ACCOUNT_… 之前），
      //    断言就会拿错账户比对而假红，而钱其实入对了账户。
      const [r] = await pool.query(
        'SELECT account_id, current_balance FROM finance_accounts WHERE account_type=? ORDER BY created_at LIMIT 1',
        [type]
      );
      return r.length ? { id: r[0].account_id, v: Number(r[0].current_balance) } : null;
    };
    const txOf = async orderId =>
      (
        await pool.query(
    ...
    const qty1 = 2;
    const expect1 = round2((1.2 + 0.5) * qty1);
    const c1 = await req('POST', '/orders', {
      token,
      body: {
        orderType: 1,
        customerName: ORDER_TAG + '客户A',
        customerPhone: '13900000001',
        customerAddress: '冒烟地址',
        deliveryMethod: 1,
        deliveryStaffId: worker.worker_id,
    ...
    ok(`类型1 农夫上单账户 +${expect1}`, nfsd1.v === round2(startNfsd.v + expect1), {
      before: startNfsd.v,
      after: nfsd1.v
    });
    ...
    const qty5 = 3;
    const expect5 = round2(20.0 * qty5);
    const c5 = await req('POST', '/orders', {
      token,
      body: {
        orderType: 5,
        customerName: ORDER_TAG + '客户B',
    ...
    const expectDeduct = round2((1.2 + 0.5) * 2);
    const expectWhole = round2(8.0 * 1);
    const c2 = await req('POST', '/orders', {
      token,
      body: {
        orderType: 2,
        stationId: STATION_ID,
        customerName: ORDER_TAG + '水站',
    ...
    ok(
      `类型2 抵扣笔=${expectDeduct}（农夫上单账户）`,
      deductTx && Number(deductTx.amount) === expectDeduct && deductTx.account_id === startNfsd.id,
      t2
    );
    ok(
      `类型2 分销笔=${expectWhole}（晟之溪公户）`,
      wholeTx && Number(wholeTx.amount) === expectWhole && wholeTx.account_id === startSzx.id,
      t2
    );
    ...
    const expectAllDeduct = round2((1.2 + 0.5) * 2);
    const c3b = await req('POST', '/orders', {
      token,
      body: {
        orderType: 2,
        stationId: STATION_ID,
        customerName: ORDER_TAG + '全抵扣水站',
    ...
    const c4 = await req('POST', '/orders', {
      ...
      body: {
        orderType: 4,
        machineStationId: 'SMKM' + suffix,
        customerName: ORDER_TAG + '机台',
        deliveryMethod: 1,
    ...
    const qtyEdit = 4;
    const expectEdit = round2((1.2 + 0.5) * qtyEdit);
    const u1 = await req('PUT', '/orders/' + oid1, {
      token,
      body: {
        orderType: 1,
        customerName: ORDER_TAG + '客户A',
    ...
    ok('改单后余额 = 原值 - 旧账 + 新账', afterEdit.v === round2(beforeEdit.v - expect1 + expectEdit), {
      before: beforeEdit.v,
      after: afterEdit.v,
      expect: round2(beforeEdit.v - expect1 + expectEdit)
    });
    ...
    ok('取消后余额回到入账前', afterCancel.v === round2(beforeCancel.v - expect5), {
      before: beforeCancel.v,
      after: afterCancel.v
    });
    ...
    if (d2.json?.code === 200) {
      ...
      ok('硬删除后余额回冲', afterDel.v === round2(beforeDel.v - expectEdit), {
        before: beforeDel.v,
        after: afterDel.v
      });
    }
    ...
    try {
      ...
      const [txs] = await pool.query(
        "SELECT tx_id, account_id, amount FROM finance_transactions WHERE related_module='order_revenue'"
      );
      for (const t of txs) {
