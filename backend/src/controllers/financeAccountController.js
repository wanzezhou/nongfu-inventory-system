...
const ACCOUNT_TYPES = {
  ...
  8: '农夫上单账户', 9: '量贩机账户', 10: '零售机账户'
};

function genAccountId() {
  return 'ACC' + Date.now().toString(36).toUpperCase() + Math.floor(Math.random() * 10000).toString(36).toUpperCase();
}
function genTxNo() {
  const d = new Date();
...
async function getAccounts(req, res) {
  try {
    const [rows] = await pool.query('SELECT * FROM finance_accounts ORDER BY account_type, created_at');
...
async function createAccount(req, res) {
  try {
    ...
    const [exist] = await pool.query('SELECT account_id FROM finance_accounts WHERE account_name = ?', [String(accountName).trim()]);
    ...
    await pool.query(
      `INSERT INTO finance_accounts (account_id, account_name, account_type, bank_name, bank_account, initial_balance, current_balance, remark, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, NOW(), NOW())`,
      [accountId, String(accountName).trim(), type, bankName || null, bankAccount || null, initial, initial, remark || null]
    );
...
async function deleteAccount(req, res) {
  try {
    const { id } = req.params;
    const [acc] = await pool.query('SELECT account_id, current_balance FROM finance_accounts WHERE account_id = ?', [id]);
...
async function insertTx(conn, { account, txType, amount, before, txNo, batchId, counterparty, remark, user }) {
  ...
  await conn.query(
    `INSERT INTO finance_transactions
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURDATE(), ?, ?, ?, NOW())`,
    [
      genTxId(), txNo, account.account_id, account.account_name, txType,
      txType === 1 ? '收入' : '支出', amount, before, after,
      'account_transfer', batchId, user || null, counterparty || null, remark || null
    ]
  );
  return after;
}
...
async function adjustBalance(req, res) {
  try {
    ...
    try {
      ...
      const [acc] = await conn.query('SELECT * FROM finance_accounts WHERE account_id = ? FOR UPDATE', [id]);
      if (!acc.length) { await conn.rollback(); return error(res, '账户不存在', 404); }
      if (Number(acc[0].status) !== 1) { await conn.rollback(); return error(res, '账户已停用，不能调账', 400); }
      const balance = Number(acc[0].current_balance);
      if (type === 'expense' && balance < amt) { await conn.rollback(); return error(res, '可用余额不足', 400); }
      const txType = type === 'income' ? 1 : 2;
      const newBalance = txType === 1 ? balance + amt : balance - amt;
      await conn.query('UPDATE finance_accounts SET current_balance = ?, updated_at = NOW() WHERE account_id = ?', [newBalance, id]);
      await insertTx(conn, { account: acc[0], txType, amount: amt, before: balance, txNo: genTxNo(), batchId: 'MANUAL', counterparty: type === 'income' ? '人工收入' : '人工支出', remark: remark || null, user });
      await conn.commit();
...
// 账户间转账（任一转出/转入方；事务：双边余额 + 双流水同一批次号）
async function transferBetween(req, res) {
  try {
    const { fromId, toId, amount, remark } = req.body;
    if (!fromId || !toId) return error(res, '请选择转出与转入账户', 400);
    if (fromId === toId) return error(res, '转出与转入账户不能相同', 400);
    const amt = Number(amount);
    if (!amount || isNaN(amt) || amt <= 0) return error(res, '金额必须为大于 0 的数字', 400);
    const user = (req.user && (req.user.username || req.user.id)) || null;
    const conn = await pool.getConnection();
    try {
      await conn.beginTransaction();
      const [fromRows] = await conn.query('SELECT * FROM finance_accounts WHERE account_id = ? FOR UPDATE', [fromId]);
      const [toRows] = await conn.query('SELECT * FROM finance_accounts WHERE account_id = ? FOR UPDATE', [toId]);
      if (!fromRows.length || !toRows.length) { await conn.rollback(); return error(res, '账户不存在', 404); }
      if (Number(fromRows[0].status) !== 1) { await conn.rollback(); return error(res, '转出账户已停用', 400); }
      if (Number(toRows[0].status) !== 1) { await conn.rollback(); return error(res, '转入账户已停用', 400); }
      const fromBalance = Number(fromRows[0].current_balance);
      if (fromBalance < amt) { await conn.rollback(); return error(res, '转出账户可用余额不足', 400); }
      const toBalance = Number(toRows[0].current_balance);
      const txNo = genTxNo();
      const batchId = 'XFER' + txNo;
      const fromAfter = await insertTx(conn, { account: fromRows[0], txType: 2, amount: amt, before: fromBalance, txNo, batchId, counterparty: toRows[0].account_name, remark: remark || null, user });
      const toAfter = await insertTx(conn, { account: toRows[0], txType: 1, amount: amt, before: toBalance, txNo, batchId, counterparty: fromRows[0].account_name, remark: remark || null, user });
      await conn.query('UPDATE finance_accounts SET current_balance = ?, updated_at = NOW() WHERE account_id = ?', [fromAfter, fromId]);
      await conn.query('UPDATE finance_accounts SET current_balance = ?, updated_at = NOW() WHERE account_id = ?', [toAfter, toId]);
      await conn.commit();
      return success(res, { txNo, fromBalance: fromAfter, toBalance: toAfter }, '转账成功');
    } catch (e) {
      await conn.rollback();
      throw e;
    } finally {
...
async function getTransactions(req, res) {
  try {
    ...
    if (endDate) { parts.push('tx_date <= ?'); params.push(endDate); }
    if (type && ['1', '2', '3', '4'].includes(String(type))) { parts.push('tx_type = ?'); params.push(Number(type)); }
