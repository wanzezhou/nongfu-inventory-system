...
async function exportData(req, res) {
  ...
  try {
    ...
    if (moduleName === 'orders') {
      ...
      `);
      const orderTypeMap = { 1: '线上平台销售', 2: '线下水站分销', 3: '线下零售', 4: '零售机供货', 5: '线下水站返货' };
...
async function importData(req, res) {
  ...
  try {
    ...
    // 特殊处理订单模块
    if (moduleName === 'orders') {
      const orderTypeMap = { '线上平台销售': 1, '线下水站分销': 2, '线下零售': 3, '零售机供货': 4, '线下水站返货': 5 };
...
async function downloadTemplate(req, res) {
  ...
  if (moduleName === 'orders') {
    ...
    const sample = ['', '线上平台销售', '张三', '13800138000', '南京市XX区XX路', 100.00, 10.00, 110.00, '自有员工配送', '备注信息', '管理员'];
    const aoa = [headers, sample];
    const ws = XLSX.utils.aoa_to_sheet(aoa);
    ws['!cols'] = headers.map(() => ({ wch: 15 }));
    const wb = XLSX.utils.book_new();
