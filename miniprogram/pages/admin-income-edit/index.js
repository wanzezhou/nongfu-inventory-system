// 收入 · 新增 / 编辑（管理员 · Phase 8b 第 2 域）
// ===========================================================================
// ⚠️ 四条要点：
//
//   ① **幂等键在「用户点保存的那一刻」生成并持久化**（§23.1），重试复用同一个键。
//      这是移动端最容易被忽略的一环：弱网下用户会连点两次「保存」，
//      没有幂等键就会生成两笔收入，而两笔都合法、账面上看不出任何异常。
//      保存期间按钮同时置灰（双保险）。
//
//   ② **金额与日期必须与后端校验口径一致**：金额 > 0、日期 YYYY-MM-DD。
//      前端校验只是为了少一次往返，**不是安全边界** —— 后端 validateBody 才是。
//
//   ③ 账户可留空 = 只登记台账不动账（后端 applyIncomeLedger 在 account_id 为空时直接返回）。
//      编辑/删除都会由后端**撤销旧流水再重记**，前端不做任何调账。
//
//   ④ 删除要二次确认：删收入会**扣回账户余额**（这笔钱没进来），是不可逆的资金动作。
//      ⚠️ 与「删除支出」方向**相反**（那个是回补余额）—— 别照抄支出页的那句话。
// ===========================================================================
...
Page({
  ...
  onLoad(query) {
    ...
    if (role !== 'admin') {
      this.setData({ loadError: '当前身份不是管理员，无权操作收入台账' });
      return;
    }

    const incomeId = (query && query.id) || '';
    this.setData({
    ...
    this.loadOptions().then(() => {
      if (incomeId) this.loadRecord(incomeId);
    });
  },
  ...
  async loadOptions() {
    try {
      const res = await ui.request.get('/admin/incomes/options', null, { silent: true });
  ...
  async loadRecord(incomeId) {
    try {
      const r = await ui.request.get(`/admin/incomes/${incomeId}`, null, { silent: true });
      ...
      this.setData({
        ...
        incomeDate: r.incomeDate || todayStr(),
        category: r.category || '',
        remark: r.remark || '',
        accountIndex,
  ...
  onNameInput(e) {
    this.setData({ incomeName: e.detail.value });
  },
  onAmountInput(e) {
  ...
  onDateChange(e) {
    this.setData({ incomeDate: e.detail.value });
  },
  onCategoryChange(e) {
  ...
  validate() {
    const name = String(this.data.incomeName || '').trim();
    if (!name) return '收入名称不能为空';
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(this.data.incomeDate)) return '日期格式应为 YYYY-MM-DD';
    if (!String(this.data.category || '').trim()) return '请选择收入类别';
    return '';
  },
  ...
  buildPayload() {
    ...
    return {
      incomeName: String(this.data.incomeName || '').trim(),
  ...
  async onSubmit() {
    ...
    const scope = this.data.isEdit ? 'UPDATE_INCOME' : 'CREATE_INCOME';
    // 幂等键：同一份内容的重试复用同一个键；内容改了则换新键（否则后端判「同键不同参数」而 400）
    const keyPayload = this.data.isEdit ? Object.assign({ id: this.data.incomeId }, payload) : payload;
    const clientRequestId = idem.acquireKey({ scope, ownerKey, payload: keyPayload });

    this.setData({ submitting: true });
    try {
      if (this.data.isEdit) {
        await ui.request.put(`/admin/incomes/${this.data.incomeId}`, Object.assign({ clientRequestId }, payload), {
          silent: true
        });
      } else {
        await ui.request.post('/admin/incomes', Object.assign({ clientRequestId }, payload), { silent: true });
      }
      idem.releaseKey();
  ...
  async onDelete() {
    if (this.data.submitting) return;
    const ok = await ui.confirm('删除这笔收入？', '删除会同时撤销该账户的资金流水并扣回余额，操作不可撤销。', '删除');
    if (!ok) return;
    ...
    try {
      ...
      await ui.request.del(
        `/admin/incomes/${this.data.incomeId}?clientRequestId=${encodeURIComponent(clientRequestId)}`,
        null,
        { silent: true }
      );
      idem.releaseKey();
