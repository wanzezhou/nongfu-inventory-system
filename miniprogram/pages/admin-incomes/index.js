...
Page({
  ...
  onLoad() {
    ...
    this.setData({
      ...
      loadError: role === ROLES.ADMIN ? '' : '当前身份不是管理员，无权查看收入台账'
    });
  },
  ...
  async fetch(reset) {
    ...
    try {
      const res = await ui.request.get('/admin/incomes', query, { silent: true });
      const shaped = (res.list || []).map(it => ({
        incomeId: it.incomeId,
        incomeName: it.incomeName,
      ...
      this.setData({ loading: false, loadError: e.message || '收入台账加载失败' });
    }
  },
  ...
  onItemTap(e) {
    ...
    if (!id) return;
    ui.navTo(`/pages/admin-income-edit/index?id=${id}`);
  },

  onCreate() {
    ui.navTo('/pages/admin-income-edit/index');
  }
});
