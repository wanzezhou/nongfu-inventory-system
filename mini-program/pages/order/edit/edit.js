...
Page({
  ...
  onLoad(options) {
    ...
    this.setData({ id: options.id || '', role, orderTypes });
    this._loadStations().then(() => this._loadWorkers()).then(() => this._loadDetail());
  },

  _loadStations() {
    return request.get('/mini/orders/stations')
      .then(list => this.setData({ stations: list || [] }))
      .catch(() => {});
  },

  _loadWorkers() {
    return request.get('/mini/orders/workers')
  ...
  _loadDetail() {
    ...
    request.get('/mini/orders/' + this.data.id)
      .then((order) => {
        wx.hideLoading();
        this._fillDetail(order);
      })
      .catch(() => wx.hideLoading());
  },

  _fillDetail(order) {
    const ot = Number(order.orderType);
    const deliveryTypes = DELIVERY_TYPES[ot] || [];
    const typeIndex = this.data.orderTypes.findIndex(t => t.value === ot);
    const dt = Number(order.deliveryType);
    const dtIndex = deliveryTypes.findIndex(t => t.value === dt);
    const stIndex = this.data.stations.findIndex(s => String(s.id) === String(order.stationId));
    const wkIndex = this.data.workers.findIndex(w => String(w.id) === String(order.workerId));
    ...
    const orderItems = (order.items || []).map(it => ({
      ...
      stock: 0,
      workerRetailDeliveryFee: Number(it.workerRetailDeliveryFee) || 0,
      workerWholesaleDeliveryFee: Number(it.workerWholesaleDeliveryFee) || 0,
      workerMachineDeliveryFee: Number(it.workerMachineDeliveryFee) || 0,
      totalDeliveryFee: Number(it.totalDeliveryFee) || 0,
    ...
    this.setData({
      ...
      stationId: order.stationId ? String(order.stationId) : '',
      stationIndex: stIndex,
      customerName: order.customerName || '',
      customerPhone: order.customerPhone || '',
      customerAddress: order.customerAddress || '',
      orderItems,
      goodsAmount: fmt.formatAmount(order.orderAmount),
      deliveryFee: fmt.formatAmount(order.deliveryFee),
      totalAmount: fmt.formatAmount(order.totalAmount),
      remark: order.remark || '',
      deliveryType: dt,
      deliveryTypeIndex: dtIndex,
      deliveryLabel: dtIndex > -1 ? deliveryTypes[dtIndex].label : '',
      needWorker: dt !== 3,
      workerId: order.workerId ? String(order.workerId) : '',
      workerIndex: wkIndex
    });
  },
  ...
  submit() {
    ...
    request.put('/mini/orders/' + this.data.id, payload)
      .then(() => {
        wx.hideLoading();
